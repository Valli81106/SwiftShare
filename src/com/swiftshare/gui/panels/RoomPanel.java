...
public class RoomPanel extends JPanel {
    ...
    public void updateTransferProgress(TransferStatus status) {
        SwingUtilities.invokeLater(() -> {
            ...
            final ProgressCard finalProgressCard;

            if (progressCard == null) {
                finalProgressCard = new ProgressCard(status);
                activeTransfers.put(fileId, finalProgressCard);
                filesPanel.add(finalProgressCard);
                filesPanel.add(Box.createVerticalStrut(10));
            } else {
                finalProgressCard = progressCard;
                finalProgressCard.updateStatus(status);
            }
            ...
            if (status.isComplete()) {
                Timer timer = new Timer(3000, e -> {
                    filesPanel.remove(finalProgressCard);
